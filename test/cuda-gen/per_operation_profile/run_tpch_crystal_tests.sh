...
for QUERY in "${QUERIES[@]}"; do
  ...
  RUN_SQL="$BUILD_DIR/run-sql $TPCH_DIR/$QUERY.sql $TPCH_DATA_DIR --gen-cuda-crystal-code --gen-per-operation-profile"
  echo $RUN_SQL
  $RUN_SQL > /dev/null 

  CODEFILE=$QUERY.crystal.peropprofile
  # Now run the generated CUDA code
  CP_CMD="cp output.cu $SQL_PLAN_COMPILER_DIR/gpu-db/tpch/q$CODEFILE.codegen.cu"
  echo $CP_CMD
  $CP_CMD
  ...
  MAKE_QUERY="make query Q=$CODEFILE CUCO_SRC_PATH=$CUCO_SRC_PATH"
  echo $MAKE_QUERY
  $MAKE_QUERY

  RUN_QUERY_CMD="build/dbruntime --data_dir $TPCH_DATA_DIR/ --query_num $CODEFILE"
  echo $RUN_QUERY_CMD
  
  $RUN_QUERY_CMD > "per_operation_data/stack_$CODEFILE.txt"

  pushd per_operation_data
  python3 ../../make-chart.py stack_$CODEFILE.txt
  popd  
