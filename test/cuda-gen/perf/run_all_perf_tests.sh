...
RUN_HYPER_PERF_SCRIPT="$SCRIPT_DIR/run_hyper_perf_tests.sh"

# Check if the run_hyper_perf_tests.sh script exists
if [ ! -f "$RUN_HYPER_PERF_SCRIPT" ]; then
  echo "Error: run_hyper_perf_tests.sh not found at $RUN_HYPER_PERF_SCRIPT"
  exit 1
fi

RUN_CRYSTAL_PERF_SCRIPT="$SCRIPT_DIR/run_crystal_perf_tests.sh"
# Check if the run_crystal_perf_tests.sh script exists
if [ ! -f "$RUN_CRYSTAL_PERF_SCRIPT" ]; then
  echo "Error: run_crystal_perf_tests.sh not found at $RUN_CRYSTAL_PERF_SCRIPT"
  exit 1
fi
