...
std::string translateConstantOp(db::ConstantOp& constantOp, bool useEncodedCompare) {
   std::string result = "";
   ...
   if (mlir::isa<db::DateType>(ty)) {
      ...
      if (useEncodedCompare) {
         auto encodedStrAttr = mlir::dyn_cast_or_null<mlir::IntegerAttr>(constantOp.getValue());
         assert(encodedStrAttr != 0x0 && "Expected encoded string constant to be an integer");
         return std::to_string(encodedStrAttr.getInt());
      }
      auto strAttr = mlir::dyn_cast_or_null<mlir::StringAttr>(constantOp.getValue());
...
bool gUseEncodedPredicates = false;
...
void checkForCodeGenSwitches(int& argc, char** argv) {
   std::tuple<bool*, std::string, std::string> switches[]  = {
      ...
      std::make_tuple(&gUseEncodedPredicates, "--use-encoded-predicates", "use encoded predicates in comparisons"),
   };
