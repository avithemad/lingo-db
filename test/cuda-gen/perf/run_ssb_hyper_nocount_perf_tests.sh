...
OUTPUT_FILE=$SCRIPT_DIR/ssb-hyper-nocount-perf.csv
...
for QUERY in "${QUERIES[@]}"; do
  ...
  COMPILE_SQL="$BUILD_DIR/run-sql $SSB_DIR/$QUERY.sql $SSB_DATA_DIR --gen-cuda-code-no-count --ssb --gen-kernel-timing"
  echo $COMPILE_SQL
  $COMPILE_SQL
