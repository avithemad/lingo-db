...
if [ $SKIP_GEN -eq 0 ]; then
  ...
  rm -f $CUDA_GEN_DIR/*.codegen.cu

  # generate new files
  FILE_SUFFIX=".crystal"
  GEN_CUDF="$BUILD_DIR/gen-cuda $DATA_DIR --gen-cuda-crystal-code $CODEGEN_OPTIONS"
...
RUN_QUERY_CMD="build/dbruntime --data_dir $DATA_DIR/ --query_num $QUERIES_STR"
echo $RUN_QUERY_CMD
$RUN_QUERY_CMD
...
for QUERY in "${QUERIES[@]}"; do
  ...
  PYTHON_CMD="python $SCRIPT_DIR/compare_tpch_outputs.py $SCRIPT_DIR/$OUTPUT_FILE $CUDA_GEN_DIR/cuda-$BENCHMARK_NAME-$QUERY.crystal.csv"
  echo $PYTHON_CMD
  $PYTHON_CMD
