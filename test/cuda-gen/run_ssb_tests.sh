...
for QUERY in "${QUERIES[@]}"; do
  ...
  RUN_SQL="$BUILD_DIR/run-sql $SSB_DIR/$QUERY.sql $SSB_DATA_DIR --gen-cuda-code --ssb"
  ...
  PYTHON_CMD="python $SCRIPT_DIR/compare_tpch_outputs.py $OUTPUT_FILE $SQL_PLAN_COMPILER_DIR/gpu-db/ssb/cuda-ssb-$QUERY.csv"
  echo $PYTHON_CMD
  $PYTHON_CMD
