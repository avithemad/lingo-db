...
OUTPUT_DIR=$SQL_PLAN_COMPILER_DIR/reports/ncu/$CUR_GPU/$BENCHMARK_NAME-$SCALE_FACTOR-crystal/$SUB_DIR
mkdir -p $OUTPUT_DIR
OUTPUT_FILE=$OUTPUT_DIR/$BENCHMARK_NAME-$SCALE_FACTOR-crystal$SUFFIX-perf.csv
...
if [ $SKIP_GEN -eq 0 ]; then
  FILE_SUFFIX=".crystal"
  GEN_CUDF="$BUILD_DIR/gen-cuda $DATA_DIR --gen-cuda-crystal-code --gen-kernel-timing $CODEGEN_OPTIONS"
...
RUN_QUERY_CMD="build/dbruntime --data_dir $DATA_DIR/ --query_num $QUERIES_STR --op_file $OUTPUT_FILE --scale_factor $SCALE_FACTOR $CONTINUOUS_ARG"
echo $RUN_QUERY_CMD
$RUN_QUERY_CMD
