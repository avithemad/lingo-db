...
for QUERY in "${QUERIES[@]}"; do
  ...
  RUN_SQL="$BUILD_DIR/run-sql $TPCH_DIR/$QUERY.sql $TPCH_DATA_DIR --gen-cuda-crystal-code-no-count --gen-per-operation-profile"
  echo $RUN_SQL
  $RUN_SQL > /dev/null 

  CODEFILE=$QUERY.crystal.nocount.peropprofile
  ...
  echo $RUN_QUERY_CMD
  $RUN_QUERY_CMD > "stack_$CODEFILE.txt"
